...
async function getAuthorizedClient(request: Request) {
  ...
  if (!token) {
    return { errorResponse: NextResponse.json({ message: 'Nao autenticado.' }, { status: 401 }) };
  }
  ...
  if (error || !data.user) {
    return { errorResponse: NextResponse.json({ message: 'Sessao do cliente invalida.' }, { status: 401 }) };
  }
  ...
  return { supabase, userId: data.user.id, clientRow };
}
